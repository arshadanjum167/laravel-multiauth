       ...
       $validation = $this->validate($request, [
        ...
        if(isset($data) && $data != array())
        {
            ...
            if($result_code==803 || $result_code==802 || $result_code==801)
            {
                ...
                return $this->ErrorResponse('',200,[],200,'account_deactivate');
            ...
            return $this->SuccessResponse($result,200,[],0,'welcome_login');
           
        }
        else
        {
            return $this->ErrorResponse('',200,[],200,'invalid_login_1');
        ...
        if (apifunction::CheckAuthToken($request)) {
            if (apifunction::checkIsPost($required_objs, $_POST)) {
                ...
                if(!apifunction::checkLoggeduser(Auth::user()->access_token))
                    return $this->ErrorResponse('',200,[],200,'error_user_have_not_access');
                ...
                if(isset($data) && $data != array())
                {
                    ...
                    return $this->SuccessResponse(array(),200,[],0,'success_logout');
                }
                else{
                    //return Yii::$app->apifunction->setResponse('error_user_not_found',false,200,400);
                    return $this->ErrorResponse('',200,[],200,'error_user_not_found');

                }
            } else {
                //return Yii::$app->apifunction->setResponse('response_text',false,200,400);
                return $this->ErrorResponse('',200,[],200,'response_text',400);
            }
        }else{
            //return Yii::$app->apifunction->setResponse('response_text',false,200,400);
            return $this->ErrorResponse('',200,[],200,'response_text',400);
    ...
    public function Register(Request $request)
    {
        ...
        if(apifunction::checkIsPost($required_objs,$_POST))
        {
            ...
            if($data == null)
            {
                ...
                if(isset($_POST['country_code']) && $_POST['country_code']!=null && isset($_POST['contact_number']) && $_POST['contact_number']!=null  )
                {
                    ...
                    if(isset($contact_data) && $contact_data != array())
                        return $this->ErrorResponse('',200,[],200,'phone_exist_already');
                ...
                if($user->save())
                {
                    ...
                    return $this->SuccessResponse(array(),200,[],0,'successfully_registered');
            ...
            else
            {
                ...
                return $this->ErrorResponse('',200,[],200,'email_exist_already');
            }
        }
        else
        {
